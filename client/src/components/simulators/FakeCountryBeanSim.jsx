...
export default function FakeCountryBeanSim({ onComplete }) {
  const [formData, setFormData] = useState({});
  const [errorMsg, setErrorMsg] = useState('');
  const [submitted, setSubmitted] = useState(false);

  const fields = [{"n":"gst_number","p":"Your GSTIN / Business Tax ID","t":"text","max":20},{"n":"phone","p":"10-Digit Distributor Mobile Number","t":"tel","max":10}];
    ...
    <div style={{ background: '#1c1917', border: '1px solid #292524', borderRadius: '18px', overflow: 'hidden', fontFamily: 'var(--font-sans, sans-serif)', color: '#f5f5f4', boxShadow: '0 16px 40px rgba(0,0,0,0.6)' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #78350f, #1c1917)', padding: '1.4rem 2rem', borderBottom: '1px solid #92400e', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.2rem', fontWeight: 'bold' }}>{"☕ Country Bean Retail Expansion — Distributor Portal"}</h3>
      ...
      <div style={{ padding: '2.2rem' }}>
        <p style={{ color: '#e7e5e4', fontSize: '1rem', lineHeight: '1.65', marginBottom: '1.5rem' }}>
          {"We have evaluated your business profile and selected you as our primary wholesale distributor for your city. You will receive 40% margin on all instant coffee and brewing accessories."}
        </p>
        <div style={{ background: '#292524', borderLeft: '4px solid #f59e0b', padding: '1.4rem', borderRadius: '8px', color: '#fef3c7', fontSize: '0.95rem', marginBottom: '2rem', whiteSpace: 'pre-wrap' }}>
          {"Onboarding Requirement: Transfer ₹75,000 refundable inventory security deposit to initiate stock dispatch."}
        </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.8rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.4rem' }}>
            ...
            >
              ...
              <span>{"Contact Brand Headquarters Directly on Verified Number"}</span>
            </button>
            ...
            >
              <span style={{ fontSize: '1.2rem' }}>⚠️</span>
              <span>{"Transfer ₹75,000 Inventory Security Deposit"}</span>
            </button>
          </div>
        </div>
      </div>
