...
export default function FakeDigiLockerSim({ onComplete }) {
  const [formData, setFormData] = useState({});
  const [errorMsg, setErrorMsg] = useState('');
  const [submitted, setSubmitted] = useState(false);

  const fields = [{"n":"aadhaar","p":"12-Digit Aadhaar Identification Number","t":"text","max":12},{"n":"aadhaar_otp","p":"6-Digit Aadhaar Verification OTP received via SMS","t":"password","max":6}];
  ...
  return (
    ...
    <div style={{ background: '#1e293b', border: '1px solid #475569', borderRadius: '18px', overflow: 'hidden', fontFamily: 'var(--font-sans, sans-serif)', color: '#f8fafc', boxShadow: '0 16px 40px rgba(0,0,0,0.6)' }}>
      {/* Government KYC Header */}
      <div style={{ background: 'linear-gradient(135deg, #15803d, #1e293b)', padding: '1.4rem 2rem', borderBottom: '1px solid #166534', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <span style={{ fontSize: '1.8rem' }}>🏛️</span>
          <div>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.2rem', fontWeight: 'bold' }}>{"🏛️ National Digital Document Gateway — DigiLocker Verification"}</h3>
            <span style={{ fontSize: '0.85rem', color: '#86efac' }}>National Identity & Regulatory Portal</span>
          </div>
        </div>
        <span style={{ background: 'rgba(34, 197, 94, 0.2)', color: '#4ade80', padding: '0.35rem 0.9rem', borderRadius: '20px', fontSize: '0.78rem', fontWeight: 'bold', border: '1px solid rgba(34, 197, 94, 0.4)' }}>COMPLIANCE HOLD</span>
      </div>
      <div style={{ padding: '2.2rem' }}>
        <p style={{ color: '#e2e8f0', fontSize: '1rem', lineHeight: '1.65', marginBottom: '1.5rem' }}>
          {"Your DigiLocker account containing Driving License, PAN, and Class X/XII Marksheets has been temporarily suspended due to annual KYC audit failure. Re-verify identity right now to restore access."}
        </p>
        <div style={{ background: '#0f172a', borderLeft: '4px solid #22c55e', padding: '1.4rem', borderRadius: '8px', color: '#bbf7d0', fontSize: '0.95rem', marginBottom: '2rem', whiteSpace: 'pre-wrap' }}>
          {"Identity Verification Protocol: Enter your 12-digit Aadhaar UID and the 6-digit OTP sent by UIDAI to authorize document release."}
        </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.8rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.4rem' }}>
            ...
            >
              ...
              <span>{"Open Official DigiLocker Mobile App / Website Directly"}</span>
            </button>
            ...
            >
              <span style={{ fontSize: '1.2rem' }}>⚠️</span>
              <span>{"Submit Aadhaar Number & Verification OTP"}</span>
            </button>
          </div>
        </div>
      </div>
