...
export default function FakeCompTIAExamSim({ onComplete }) {
  const [formData, setFormData] = useState({});
  const [errorMsg, setErrorMsg] = useState('');
  const [submitted, setSubmitted] = useState(false);

  const fields = [{"n":"email","p":"Your Registered Pearson VUE Email","t":"text","max":40},{"n":"card_details","p":"Card Number & CVV (for $99 checkout)","t":"password","max":25}];
    ...
    <div style={{ background: '#1e1b4b', border: '1px solid #312e81', borderRadius: '18px', overflow: 'hidden', fontFamily: 'var(--font-sans, sans-serif)', color: '#f5f3ff', boxShadow: '0 16px 40px rgba(0,0,0,0.6)' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #4338ca, #1e1b4b)', padding: '1.4rem 2rem', borderBottom: '1px solid #4f46e5', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.2rem', fontWeight: 'bold' }}>{"🎟️ Global IT Certification Vouchers — CompTIA Flash Sale"}</h3>
      ...
      <div style={{ padding: '2.2rem' }}>
        <p style={{ color: '#ddd6fe', fontSize: '1rem', lineHeight: '1.65', marginBottom: '1.5rem' }}>
          {"Get certified for less! Buy official CompTIA Security+ (SY0-701) exam vouchers valid at any Pearson VUE testing center worldwide for just $99 (Official Price: $392)."}
        </p>
        <div style={{ background: '#312e81', border: '1px solid #4338ca', padding: '1.4rem', borderRadius: '12px', color: '#fbcfe8', fontSize: '0.95rem', marginBottom: '2rem', whiteSpace: 'pre-wrap' }}>
          {"Delivery: Instant voucher code delivery via email upon cryptocurrency or debit card payment check-out."}
        </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.8rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.4rem' }}>
            ...
            >
              ...
              <span>{"Buy Vouchers Only from Official CompTIA / Pearson VUE Store"}</span>
            </button>
            ...
            >
              <span style={{ fontSize: '1.2rem' }}>⚠️</span>
              <span>{"Pay $99 for Discounted Certification Voucher"}</span>
            </button>
          </div>
        </div>
      </div>
