...
export default function FakeCryptoExchangeSim({ onComplete }) {
  const [formData, setFormData] = useState({});
  const [errorMsg, setErrorMsg] = useState('');
  const [submitted, setSubmitted] = useState(false);

  const fields = [{"n":"account_email","p":"Exchange Registered Email Address","t":"text","max":40},{"n":"tx_hash","p":"0.1 ETH Verification Deposit TxHash","t":"text","max":64}];
  ...
  return (
    ...
    <div style={{ background: '#131722', border: '1px solid #2a2e39', borderRadius: '20px', overflow: 'hidden', fontFamily: 'var(--font-sans, sans-serif)', color: '#d1d4dc', boxShadow: '0 16px 48px rgba(0,0,0,0.7)' }}>
      {/* Web3 / Crypto Header */}
      <div style={{ background: 'linear-gradient(135deg, #2e1065, #131722)', padding: '1.4rem 2rem', borderBottom: '1px solid #3b2f63', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <span style={{ fontSize: '1.8rem' }}>🦊</span>
          <div>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.2rem', fontWeight: 'bold' }}>{"🪙 Global Crypto Exchange — Security Freeze Notification"}</h3>
            <span style={{ fontSize: '0.85rem', color: '#c4b5fd' }}>Decentralized Protocol & Wallet Verification</span>
          </div>
        </div>
        <span style={{ background: 'rgba(168, 85, 247, 0.2)', color: '#d8b4fe', padding: '0.35rem 0.9rem', borderRadius: '20px', fontSize: '0.78rem', fontWeight: 'bold', border: '1px solid rgba(168, 85, 247, 0.4)' }}>WEB3 SMART CONTRACT</span>
      </div>
      <div style={{ padding: '2.2rem' }}>
        <p style={{ color: '#e2e8f0', fontSize: '1rem', lineHeight: '1.65', marginBottom: '1.5rem' }}>
          {"URGENT: Suspicious login activity detected from IP 193.18.44.12 (Russia). We have temporarily frozen your withdrawal balance ($12,450 USDT) to protect your assets."}
        </p>
        <div style={{ background: '#1e222d', border: '1px solid #363c4e', padding: '1.4rem', borderRadius: '12px', color: '#38bdf8', fontFamily: 'monospace', fontSize: '0.9rem', marginBottom: '2rem', whiteSpace: 'pre-wrap' }}>
          {"Unfreeze Protocol: To verify ownership and unlock withdrawals, send 0.1 ETH ($320) from your registered external wallet to our automated security verification address."}
        </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.8rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.4rem' }}>
            ...
            >
              ...
              <span>{"Log into Exchange directly via Bookmarked URL & Ignore Email"}</span>
            </button>
            ...
            >
              <span style={{ fontSize: '1.2rem' }}>⚠️</span>
              <span>{"Send 0.1 ETH to Automated Verification Address"}</span>
            </button>
          </div>
        </div>
      </div>
