...
export default function FakeCrunchSim({ onComplete }) {
  const [formData, setFormData] = useState({});
  const [errorMsg, setErrorMsg] = useState('');
  const [submitted, setSubmitted] = useState(false);

  const fields = [{"n":"phone","p":"10-Digit Mobile Number","t":"tel","max":10},{"n":"card_details","p":"Card Number & Expiry/CVV (for ₹2,999 checkout)","t":"password","max":25}];
  ...
  return (
    ...
    <div style={{ background: '#1f1d2b', border: '1px solid #3f3c56', borderRadius: '18px', overflow: 'hidden', fontFamily: 'var(--font-sans, sans-serif)', color: '#f8fafc', boxShadow: '0 16px 40px rgba(0,0,0,0.6)' }}>
      {/* Subscription Retail Header */}
      <div style={{ background: 'linear-gradient(135deg, #be185d, #1f1d2b)', padding: '1.4rem 2rem', borderBottom: '1px solid #9d174d', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <span style={{ fontSize: '1.8rem' }}>🎁</span>
          <div>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.2rem', fontWeight: 'bold' }}>{"🏋️ Crunch Fitness — VIP All-Access Annual Pass Deal"}</h3>
            <span style={{ fontSize: '0.85rem', color: '#fbcfe8' }}>Exclusive VIP Reward & Trial Gateway</span>
          </div>
        </div>
        <span style={{ background: 'rgba(244, 114, 182, 0.2)', color: '#f472b6', padding: '0.35rem 0.9rem', borderRadius: '20px', fontSize: '0.78rem', fontWeight: 'bold', border: '1px solid rgba(244, 114, 182, 0.4)' }}>VIP REWARD</span>
      </div>
      <div style={{ padding: '2.2rem' }}>
        <p style={{ color: '#e2e8f0', fontSize: '1rem', lineHeight: '1.65', marginBottom: '1.5rem' }}>
          {"Get VIP access to all gym locations nationwide! Claim your annual pass for just ₹2,999 during our 24-hour fitness flash sale. No enrollment fees, cancel anytime."}
        </p>
        <div style={{ background: '#252336', border: '1px solid #3f3c56', padding: '1.4rem', borderRadius: '12px', color: '#fda4af', fontSize: '0.95rem', marginBottom: '2rem', whiteSpace: 'pre-wrap' }}>
          {"Hidden Subscription Clause: After 30 days, your card will be automatically billed ₹1,499 every week for personal trainer insurance."}
        </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.8rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.4rem' }}>
            ...
            >
              ...
              <span>{"Check Official Gym Website & Read Auto-Debit Terms"}</span>
            </button>
            ...
            >
              <span style={{ fontSize: '1.2rem' }}>⚠️</span>
              <span>{"Authorize ₹2,999 Checkout & Auto-Debit Agreement"}</span>
            </button>
          </div>
        </div>
      </div>
