...
export default function FakeCandySim({ onComplete }) {
  const [formData, setFormData] = useState({});
  const [errorMsg, setErrorMsg] = useState('');
  const [submitted, setSubmitted] = useState(false);

  const fields = [{"n":"phone","p":"10-Digit Mobile Number","t":"tel","max":10},{"n":"card_info","p":"Card Number & CVV (for ₹99 shipping fee)","t":"password","max":25}];
    ...
    <div style={{ background: '#1f1d2b', border: '1px solid #3f3c56', borderRadius: '18px', overflow: 'hidden', fontFamily: 'var(--font-sans, sans-serif)', color: '#f8fafc', boxShadow: '0 16px 40px rgba(0,0,0,0.6)' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #be185d, #1f1d2b)', padding: '1.4rem 2rem', borderBottom: '1px solid #9d174d', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.2rem', fontWeight: 'bold' }}>{"🍬 Gourmet Confectionery Celebration Giveaway #WIN-481"}</h3>
      ...
      <div style={{ padding: '2.2rem' }}>
        <p style={{ color: '#e2e8f0', fontSize: '1rem', lineHeight: '1.65', marginBottom: '1.5rem' }}>
          {"Congratulations! You are selected as today lucky winner of the Giant Chocolate & Candy Hamper. Just pay a nominal courier fee of ₹99 to confirm delivery address."}
        </p>
        <div style={{ background: '#252336', border: '1px solid #3f3c56', padding: '1.4rem', borderRadius: '12px', color: '#fda4af', fontSize: '0.95rem', marginBottom: '2rem', whiteSpace: 'pre-wrap' }}>
          {"Fine Print (Hidden in light gray at bottom): By paying ₹99, you enroll in a ₹3,499/month recurring VIP candy club auto-debited from your card."}
        </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.8rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.4rem' }}>
            ...
            >
              ...
              <span>{"Inspect Fine Print & Exit Sweepstakes Page"}</span>
            </button>
            ...
            >
              <span style={{ fontSize: '1.2rem' }}>⚠️</span>
              <span>{"Pay ₹99 Shipping & Authorize Recurring Debit"}</span>
            </button>
          </div>
        </div>
      </div>
