...
export default function FakeContentSamuraiSim({ onComplete }) {
  const [formData, setFormData] = useState({});
  const [errorMsg, setErrorMsg] = useState('');
  const [submitted, setSubmitted] = useState(false);

  const fields = [{"n":"email","p":"Account Registration Email Address","t":"text","max":40},{"n":"card_info","p":"Credit Card Number & CVV Code","t":"password","max":25}];
  ...
  return (
    ...
    <div style={{ background: '#0f172a', border: '1px solid #334155', borderRadius: '18px', overflow: 'hidden', fontFamily: 'var(--font-sans, sans-serif)', color: '#f8fafc', boxShadow: '0 16px 40px rgba(0,0,0,0.6)' }}>
      {/* SaaS Cloud Tools Header */}
      <div style={{ background: 'linear-gradient(135deg, #1e40af, #0f172a)', padding: '1.4rem 2rem', borderBottom: '1px solid #1e40af', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <span style={{ fontSize: '1.8rem' }}>⚙️</span>
          <div>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.2rem', fontWeight: 'bold' }}>{"🎬 Content Samurai AI — Exclusive Lifetime Licensing Deal"}</h3>
            <span style={{ fontSize: '0.85rem', color: '#93c5fd' }}>Cloud API & Workspace Permission Gateway</span>
          </div>
        </div>
        <span style={{ background: 'rgba(96, 165, 250, 0.2)', color: '#93c5fd', padding: '0.35rem 0.9rem', borderRadius: '20px', fontSize: '0.78rem', fontWeight: 'bold', border: '1px solid rgba(96, 165, 250, 0.4)' }}>OAUTH REQUEST</span>
      </div>
      <div style={{ padding: '2.2rem' }}>
        <p style={{ color: '#e2e8f0', fontSize: '1rem', lineHeight: '1.65', marginBottom: '1.5rem' }}>
          {"Never pay monthly subscription fees again! Get unlimited AI text-to-video rendering, realistic voiceovers, and commercial licensing for a single lifetime payment of $49 (Regular Price: $97/month)."}
        </p>
        <div style={{ background: '#1e293b', border: '1px solid #334155', padding: '1.4rem', borderRadius: '10px', color: '#38bdf8', fontFamily: 'monospace', fontSize: '0.9rem', marginBottom: '2rem', whiteSpace: 'pre-wrap' }}>
          {"Warning Indicator: Domain registered 3 days ago. Checkout page does not use Stripe or PayPal buyer protection."}
        </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.8rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.4rem' }}>
            ...
            >
              ...
              <span>{"Verify Software Vendor Domain & Avoid Unrealistic Lifetime Deals"}</span>
            </button>
            ...
            >
              <span style={{ fontSize: '1.2rem' }}>⚠️</span>
              <span>{"Pay $49 for Lifetime AI Video Subscription"}</span>
            </button>
          </div>
        </div>
      </div>
