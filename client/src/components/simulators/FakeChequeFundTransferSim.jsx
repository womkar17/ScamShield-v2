...
export default function FakeChequeFundTransferSim({ onComplete }) {
  const [formData, setFormData] = useState({});
  const [errorMsg, setErrorMsg] = useState('');
  const [submitted, setSubmitted] = useState(false);

  const fields = [{"n":"account_number","p":"Your 15-Digit Bank Account Number","t":"text","max":18},{"n":"phone","p":"10-Digit Registered Mobile Number","t":"tel","max":10}];
    ...
    <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '18px', overflow: 'hidden', fontFamily: 'var(--font-sans, sans-serif)', color: '#f8fafc', boxShadow: '0 16px 40px rgba(0,0,0,0.6)' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #164e63, #0f172a)', padding: '1.4rem 2rem', borderBottom: '1px solid #164e63', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.2rem', fontWeight: 'bold' }}>{"🏦 National Clearing House — Cheque Release #CHQ-5519"}</h3>
      ...
      <div style={{ padding: '2.2rem' }}>
        <p style={{ color: '#e2e8f0', fontSize: '1rem', lineHeight: '1.65', marginBottom: '1.5rem' }}>
          {"A high-value corporate dividend cheque of ₹2,50,000 is held at our clearing gateway due to interbank verification charges. Pay the clearance fee to release funds into your savings account."}
        </p>
        <div style={{ background: 'linear-gradient(145deg, #1e293b, #0f172a)', border: '1px solid #334155', padding: '1.4rem', borderRadius: '12px', color: '#fbbf24', fontWeight: '600', fontSize: '0.95rem', marginBottom: '2rem', whiteSpace: 'pre-wrap' }}>
          {"Pending Amount: ₹2,50,000 | Clearance Charge: ₹1,499\nStatus: Will be returned to sender if fee is unpaid within 24 hours."}
        </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.8rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.4rem' }}>
            ...
            >
              ...
              <span>{"Ignore SMS & Check Bank Account Summary Directly"}</span>
            </button>
            ...
            >
              <span style={{ fontSize: '1.2rem' }}>⚠️</span>
              <span>{"Pay ₹1,499 Cheque Processing Fee"}</span>
            </button>
          </div>
        </div>
      </div>
