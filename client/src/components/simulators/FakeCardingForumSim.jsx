...
export default function FakeCardingForumSim({ onComplete }) {
  const [formData, setFormData] = useState({});
  const [errorMsg, setErrorMsg] = useState('');
  const [submitted, setSubmitted] = useState(false);

  const fields = [{"n":"telegram_username","p":"Telegram Handle (@username)","t":"text","max":30},{"n":"tx_id","p":"Bitcoin Deposit Transaction Hash (TxID)","t":"text","max":64}];
    ...
    <div style={{ background: '#131722', border: '1px solid #2a2e39', borderRadius: '20px', overflow: 'hidden', fontFamily: 'var(--font-sans, sans-serif)', color: '#d1d4dc', boxShadow: '0 16px 48px rgba(0,0,0,0.7)' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #2e1065, #131722)', padding: '1.4rem 2rem', borderBottom: '1px solid #3b2f63', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.2rem', fontWeight: 'bold' }}>{"🕶️ ShadowNet VIP Carding & Dump Syndicate"}</h3>
      ...
      <div style={{ padding: '2.2rem' }}>
        <p style={{ color: '#e2e8f0', fontSize: '1rem', lineHeight: '1.65', marginBottom: '1.5rem' }}>
          {"Welcome to the private escrow portal. To gain VIP forum access and download verified high-limit cards, deposit 0.005 BTC ($350) into our automated escrow wallet."}
        </p>
        <div style={{ background: '#1e222d', border: '1px solid #363c4e', padding: '1.4rem', borderRadius: '12px', color: '#38bdf8', fontFamily: 'monospace', fontSize: '0.9rem', marginBottom: '2rem', whiteSpace: 'pre-wrap' }}>
          {"Escrow Guarantee: 100% automated verification. Once BTC is confirmed on blockchain, VIP credentials are delivered to your Telegram."}
        </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.8rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.4rem' }}>
            ...
            >
              ...
              <span>{"Avoid Underground Fraud Forums & Close Portal"}</span>
            </button>
            ...
            >
              <span style={{ fontSize: '1.2rem' }}>⚠️</span>
              <span>{"Deposit 0.005 BTC to Unlock VIP Forum Access"}</span>
            </button>
          </div>
        </div>
      </div>
