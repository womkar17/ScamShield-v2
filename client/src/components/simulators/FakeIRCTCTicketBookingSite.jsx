        ...
        <div style={{ background: "#fff", borderRadius: "20px", overflow: "hidden", boxShadow: "0 20px 50px rgba(59,130,246,.15)" }}>
          ...
          <div style={{ padding: "35px" }}>
            ...
            <div style={{ maxWidth: "600px", margin: "auto" }}>
              <input placeholder="Your Full Name" value={name} onChange={(e) => (() => { const v = e.target.value.replace(/[^a-zA-Z\s.-]/g, ''); e.target.value = v; setName(v); })()} style={inputStyle} />
              <input placeholder="Mobile Number (for PNR SMS)" value={mobile} onChange={(e) => (() => { const v = e.target.value.replace(/\D/g, '').slice(0, 15); e.target.value = v; setMobile(v); })()} style={inputStyle} />
              <input placeholder="Email Address (for ticket confirmation)" value={email} onChange={(e) => (() => { const v = e.target.value.replace(/\s/g, ''); e.target.value = v; setEmail(v); })()} style={inputStyle} />
