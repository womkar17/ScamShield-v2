...
export default function FakeCNCMachiningSim({ onComplete }) {
  const [formData, setFormData] = useState({});
  const [errorMsg, setErrorMsg] = useState('');
  const [submitted, setSubmitted] = useState(false);

  const fields = [{"n":"company_id","p":"Your Company Tax ID / PO Number","t":"text","max":30},{"n":"wire_ref","p":"Bank Wire Transfer Reference Number","t":"text","max":40}];
  ...
  return (
    ...
    <div style={{ background: '#1c1917', border: '1px solid #292524', borderRadius: '18px', overflow: 'hidden', fontFamily: 'var(--font-sans, sans-serif)', color: '#f5f5f4', boxShadow: '0 16px 40px rgba(0,0,0,0.6)' }}>
      {/* Franchise Business Header */}
      <div style={{ background: 'linear-gradient(135deg, #78350f, #1c1917)', padding: '1.4rem 2rem', borderBottom: '1px solid #92400e', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <span style={{ fontSize: '1.8rem' }}>🏢</span>
          <div>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.2rem', fontWeight: 'bold' }}>{"⚙️ Industrial Precision Engineering — Invoice #CNC-4410"}</h3>
            <span style={{ fontSize: '0.85rem', color: '#fde68a' }}>Corporate Partner Onboarding Portal</span>
          </div>
        </div>
        <span style={{ background: 'rgba(245, 158, 11, 0.2)', color: '#fbbf24', padding: '0.35rem 0.9rem', borderRadius: '20px', fontSize: '0.78rem', fontWeight: 'bold', border: '1px solid rgba(245, 158, 11, 0.4)' }}>PARTNER INVITE</span>
      </div>
      <div style={{ padding: '2.2rem' }}>
        <p style={{ color: '#e7e5e4', fontSize: '1rem', lineHeight: '1.65', marginBottom: '1.5rem' }}>
          {"We have completed raw material procurement for your custom aluminum CNC machined brackets. Production scheduled to begin immediately upon receipt of 50% advance bank wire transfer."}
        </p>
        <div style={{ background: '#292524', borderLeft: '4px solid #f59e0b', padding: '1.4rem', borderRadius: '8px', color: '#fef3c7', fontSize: '0.95rem', marginBottom: '2rem', whiteSpace: 'pre-wrap' }}>
          {"Invoice Total: $14,500 | Advance Required: $7,250\nBeneficiary Bank: Offshore Commercial Bank (Account changed from previous invoices)."}
        </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.8rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.4rem' }}>
            ...
            >
              ...
              <span>{"Call Supplier via Known Office Phone to Verify Bank Change"}</span>
            </button>
            ...
            >
              <span style={{ fontSize: '1.2rem' }}>⚠️</span>
              <span>{"Authorize $7,250 Wire Transfer to New Account"}</span>
            </button>
          </div>
        </div>
      </div>
