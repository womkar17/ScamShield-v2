...
export default function FakeCodingBootcampSim({ onComplete }) {
  const [formData, setFormData] = useState({});
  const [errorMsg, setErrorMsg] = useState('');
  const [submitted, setSubmitted] = useState(false);

  const fields = [{"n":"phone","p":"10-Digit Student Mobile Number","t":"tel","max":10},{"n":"upi_id","p":"Your UPI ID (to process ₹12,000 onboarding fee)","t":"text","max":40}];
    ...
    <div style={{ background: '#1e1b4b', border: '1px solid #312e81', borderRadius: '18px', overflow: 'hidden', fontFamily: 'var(--font-sans, sans-serif)', color: '#f5f3ff', boxShadow: '0 16px 40px rgba(0,0,0,0.6)' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #4338ca, #1e1b4b)', padding: '1.4rem 2rem', borderBottom: '1px solid #4f46e5', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.2rem', fontWeight: 'bold' }}>{"💻 NextGen Dev Bootcamp — Guaranteed Job Agreement"}</h3>
      ...
      <div style={{ padding: '2.2rem' }}>
        <p style={{ color: '#ddd6fe', fontSize: '1rem', lineHeight: '1.65', marginBottom: '1.5rem' }}>
          {"Sign up for our 6-month Full Stack AI Engineering Bootcamp! We guarantee a minimum salary package of ₹15 LPA at top tech companies. Pay zero tuition until you get hired!"}
        </p>
        <div style={{ background: '#312e81', border: '1px solid #4338ca', padding: '1.4rem', borderRadius: '12px', color: '#fbcfe8', fontSize: '0.95rem', marginBottom: '2rem', whiteSpace: 'pre-wrap' }}>
          {"Mandatory Condition: Pay ₹12,000 non-refundable \"Laptop Security & Platform Access Fee\" to begin classes."}
        </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.8rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.4rem' }}>
            ...
            >
              ...
              <span>{"Inspect Refund Policy & Reject Upfront Platform Fees"}</span>
            </button>
            ...
            >
              <span style={{ fontSize: '1.2rem' }}>⚠️</span>
              <span>{"Transfer ₹12,000 Platform Access Deposit"}</span>
            </button>
          </div>
        </div>
      </div>
