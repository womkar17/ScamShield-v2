...
export default function FakeDieterSim({ onComplete }) {
  const [formData, setFormData] = useState({});
  const [errorMsg, setErrorMsg] = useState('');
  const [submitted, setSubmitted] = useState(false);

  const fields = [{"n":"phone","p":"10-Digit Mobile Number","t":"tel","max":10},{"n":"card_info","p":"Credit Card Number & CVV (for ₹199 shipping)","t":"password","max":25}];
    ...
    <div style={{ background: '#1f1d2b', border: '1px solid #3f3c56', borderRadius: '18px', overflow: 'hidden', fontFamily: 'var(--font-sans, sans-serif)', color: '#f8fafc', boxShadow: '0 16px 40px rgba(0,0,0,0.6)' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #be185d, #1f1d2b)', padding: '1.4rem 2rem', borderBottom: '1px solid #9d174d', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.2rem', fontWeight: 'bold' }}>{"🥑 Bio-Keto Miracle Slim — Free Bottle Trial Claim"}</h3>
      ...
      <div style={{ padding: '2.2rem' }}>
        <p style={{ color: '#e2e8f0', fontSize: '1rem', lineHeight: '1.65', marginBottom: '1.5rem' }}>
          {"Based on your metabolic quiz, you qualify for a complimentary 30-day supply of Bio-Keto Slim Gummies! Just pay ₹199 for expedited courier delivery to your doorstep."}
        </p>
        <div style={{ background: '#252336', border: '1px solid #3f3c56', padding: '1.4rem', borderRadius: '12px', color: '#fda4af', fontSize: '0.95rem', marginBottom: '2rem', whiteSpace: 'pre-wrap' }}>
          {"Hidden Billing Clause: By paying ₹199 today, you consent to automatic shipments billed at ₹4,999 on the 15th of every month."}
        </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.8rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.4rem' }}>
            ...
            >
              ...
              <span>{"Read Terms and Conditions & Avoid Free Bottle Trials"}</span>
            </button>
            ...
            >
              <span style={{ fontSize: '1.2rem' }}>⚠️</span>
              <span>{"Pay ₹199 Shipping & Enroll in Auto-Shipments"}</span>
            </button>
          </div>
        </div>
      </div>
