...
export default function FakeHotelBookingSim({ onComplete }) {
  ...
  const fields = [{"n":"card_number","p":"Credit/Debit Card Number (to receive refund credit)","t":"text","max":19},{"n":"cvv_otp","p":"Card CVV & SMS Verification OTP","t":"password","max":10}];
    ...
    <div style={{ background: '#18181b', border: '1px solid #27272a', borderRadius: '18px', overflow: 'hidden', fontFamily: 'var(--font-sans, sans-serif)', color: '#f4f4f5', boxShadow: '0 16px 40px rgba(0,0,0,0.6)' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #1d4ed8, #18181b)', padding: '1.4rem 2rem', borderBottom: '1px solid #2563eb', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.2rem', fontWeight: 'bold' }}>{"🏨 Grand Palace Resort — Reservation Refund Processing"}</h3>
      ...
      <div style={{ padding: '2.2rem' }}>
        <p style={{ color: '#e4e4e7', fontSize: '1rem', lineHeight: '1.65', marginBottom: '1.5rem' }}>
          {"We have approved your refund request of ₹14,000 for your cancelled weekend stay. To credit the funds back into your account, please enter your debit/credit card number and CVV on our secure verification gateway."}
        </p>
        <div style={{ background: '#27272a', borderLeft: '4px solid #3b82f6', padding: '1.4rem', borderRadius: '8px', color: '#bfdbfe', fontSize: '0.95rem', marginBottom: '2rem', whiteSpace: 'pre-wrap' }}>
          {"Refund Amount: ₹14,000 | Gateway Protocol: Requires active card CVV and OTP to reverse payment."}
        </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.8rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.4rem' }}>
            ...
            >
              ...
              <span>{"Remember: Refunds NEVER Require CVV or OTP & Call Hotel"}</span>
            </button>
            ...
            >
              <span style={{ fontSize: '1.2rem' }}>⚠️</span>
              <span>{"Enter Card Number, CVV & OTP to Get Refund"}</span>
            </button>
          </div>
        </div>
      </div>
