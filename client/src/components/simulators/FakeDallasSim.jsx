...
export default function FakeDallasSim({ onComplete }) {
  const [formData, setFormData] = useState({});
  const [errorMsg, setErrorMsg] = useState('');
  const [submitted, setSubmitted] = useState(false);

  const fields = [{"n":"account_number","p":"Bank Checking Account Number","t":"text","max":18},{"n":"routing_number","p":"9-Digit Bank Routing Number","t":"text","max":9}];
    ...
    <div style={{ background: '#1e293b', border: '1px solid #475569', borderRadius: '18px', overflow: 'hidden', fontFamily: 'var(--font-sans, sans-serif)', color: '#f8fafc', boxShadow: '0 16px 40px rgba(0,0,0,0.6)' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #15803d, #1e293b)', padding: '1.4rem 2rem', borderBottom: '1px solid #166534', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.2rem', fontWeight: 'bold' }}>{"🏛️ City Municipal Revenue Department — Property Tax Assessment"}</h3>
      ...
      <div style={{ padding: '2.2rem' }}>
        <p style={{ color: '#e2e8f0', fontSize: '1rem', lineHeight: '1.65', marginBottom: '1.5rem' }}>
          {"URGENT NOTICE: Your property tax assessment #TX-Dallas-8819 is overdue by 30 days. To avoid a tax lien filing against your real estate, settle the balance online immediately."}
        </p>
        <div style={{ background: '#0f172a', borderLeft: '4px solid #22c55e', padding: '1.4rem', borderRadius: '8px', color: '#bbf7d0', fontSize: '0.95rem', marginBottom: '2rem', whiteSpace: 'pre-wrap' }}>
          {"Overdue Assessment: $1,450.00 | Penalty Charge: $120.00\nPayment Method: Direct bank routing check check-out."}
        </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.8rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.4rem' }}>
            ...
            >
              ...
              <span>{"Verify Tax Balance on Official City Website (.gov)"}</span>
            </button>
            ...
            >
              <span style={{ fontSize: '1.2rem' }}>⚠️</span>
              <span>{"Submit Bank Routing Details to Settle Tax Notice"}</span>
            </button>
          </div>
        </div>
      </div>
