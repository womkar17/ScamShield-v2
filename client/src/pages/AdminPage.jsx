...
export default function AdminPage() {
  ...
  const loadPhishingDrills = async () => {
    ...
    try {
      ...
      if (data.ok && Array.isArray(data.drills)) {
        const localSaved = JSON.parse(localStorage.getItem('ss_phishing_campaigns') || '[]');
        const combined = [...data.drills];
        if (Array.isArray(localSaved)) {
          localSaved.forEach(localItem => {
            if (!combined.some(c => String(c.id) === String(localItem.id))) {
        ...
        localStorage.setItem('ss_phishing_campaigns', JSON.stringify(combined));
        return;
      }
    } catch (e) {}
  ...
  const loadCustomGames = async () => {
    ...
    try {
      ...
      if (data.ok && Array.isArray(data.games)) {
        const localSaved = JSON.parse(localStorage.getItem('ss_custom_games') || '[]');
        ...
        setCustomGames(combined);
        localStorage.setItem('ss_custom_games', JSON.stringify(combined));
        return;
      }
    } catch (e) {}
  ...
  const handleAIGenerateGameContent = async () => {
    ...
    const qCount = diff === 'Hard' ? 7 : diff === 'Medium' ? 5 : 3;
    const swipeCount = diff === 'Hard' ? 12 : diff === 'Medium' ? 8 : 5;
...
Return ONLY a valid JSON object with exactly two keys:
{
  ...
  const handleLaunchCampaign = async () => {
    ...
    try {
      ...
      await supabase.from('phishing_drills').upsert([dbRow]);
    } catch (e) {
...
const s = {
  ...
  tabActive: {
    color: 'var(--text-h)',
    background: 'var(--accent-bg, rgba(168, 85, 247, 0.2))',
